...
\begin{equation}
    \mby (t_i) = g(t_i, \mbx (t_i), \mbu, \mbp, c) + \epsilon (t_i),
...
The first step is to calculate local sensitivities $s^x_{ij} = (\partial x_i / \partial p_j )$.
To do this, we need to solve the following system of the \acp{ode}:
\begin{equation}
    \begin{cases}
    \dot x_i (t) = f_i(t, \mbx, \mbu, \mbp, c)\\
...
That means that the next step is to calculate $(\partial y_i / \partial p_j) = s_{ij}$ at a certain times $t_m$ and input $u_n$ using the solutions of $s^x_{ij}$:
\begin{equation}
...
\begin{equation}
    \widetilde{s}_{ij} (t_m, u_n) = \frac{\partial y_i}{\partial p_j} \frac{p_j}{y_i}\bigg|_{t_m, u_n}
