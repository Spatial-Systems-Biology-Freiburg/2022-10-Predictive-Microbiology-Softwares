...
\begin{table}[H]
    ...
    \toprule \vspace{1mm} 
    ODE & $f$                       & \mintinline[bgcolor=white,style=emacs]{python}{def pool_model(t, y, Q, P, Const):} \\
        & $\partial f / \partial x$ & \mintinline[bgcolor=white,style=emacs]{python}{def dfdx(t, y, Q, P, Const):} \\[0.5ex]
        & $\partial f / \partial p$ & \mintinline[bgcolor=white,style=emacs]{python}{def dfdp(t, y, Q, P, Const):} \\[0.5ex]
    \midrule
    Observable  & $g$                       &  \\[0.5ex]
    (optional)  & $\partial g / \partial x$ &  \\[0.5ex]
                & $\partial g / \partial p$ &  \\[0.5ex]
    \midrule
    Time points     & $t_i$ & \\[0.5ex]
    Inputs          & $u$   & \\[0.5ex]
    Parameters      & $p$   & \\[0.5ex]
    Other arguments &       & \\[0.5ex]
    \bottomrule
    \end{tabular}
...
\begin{enumerate}
    ...
    \item Define initial values \mintinline[bgcolor=white,style=emacs]{python}{(x0, t0)}
\end{enumerate}
%
% ########################################################################
