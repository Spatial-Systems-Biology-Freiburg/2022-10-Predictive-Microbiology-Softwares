...
In the supplied toolbox {\it eDPM}, three methods were implemented: differential evolution, basin-hopping, and so-called “brute force” method.
...
This iteratite algorithm combines the Monte-Carlo and local optimisation and works as follows.
During the iteration step the design vector, i.e., vector of measurement times and inputs, is subjected to a random preturbation and then to local minimization.
...
For deeper understanding Monte-Carlo minimization the reader can refer to the papers~\cite{li_monte_1987, beichl_metropolis_2000}.
\newline
...
This method is suitable for discrete optimization with a limited number of grid values.
However, the downside of this technique is its slowness, inefficiency and long computational times that can be noticed by increasing the possible discrete solutions~\cite{scipybrute}.
\newline
