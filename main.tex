...
Another advantage is its fast convergence and robustness at numerical optimization~\cite{babu_differential_2007} (see Note~\ref{note-differential-evolution-details}).

The second implemented method is basin-hopping developed by David Wales and Jonathan Doye~\cite{wales_global_1997}.
This iterative algorithm combines Monte-Carlo and local optimization (see Note~\ref{note-basin-hopping-details}).
...
The resulting class \mintinline[bgcolor=white,style=emacs]{python}{fsr} contains all definitions, current values (see Note~\ref{note-timepoints}) and information on the optimization process.
