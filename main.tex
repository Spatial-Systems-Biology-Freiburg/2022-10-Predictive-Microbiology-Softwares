...
Using this parameter set a first optimal experimental design is determined taking into account specific constraints, such as maximum number of measurements, measurement times, etc. Next, the designed experiments need to be performed and this data results into a new estimation of the parameters.
...
These packages comprise a large number of tools and due to this require some time to understand how to use them.
To make this tutorial self-contained, we developed \mintinline[bgcolor=white,style=emacs]{bash}{eDPM} (Experimental Design for Predictive Microbiology), which is a set of tools to calculate the Sensitivity and Fischer Information Matrix and do parameter space exploration in order to find optimal results.
...
To demonstrate how Experimental Design works, we use in this tutorial the widely employed mathematical model developed by Baranyi and Roberts \cite{baranyiDynamicApproach1994}, which was devised to describe bacteria growth.
...
\begin{code}[h]
    ...
    \caption{
        ...
        We explain the individual steps needed to fully describe the model.
    }
...
\end{itemize}
The method of maximum likelihood consist in searching for the parameter vector $\mbp$ which maximizes the likelihood or equivalently minimises the log-likelihood \cite{gaborRobustEfficient2015}.
%
%
% ########################################################################
...
Due to this relationship maximization of the information leads to a minimisation of the variance or uncertainty in the parameters.
...
\begin{equation}
    F = S^T Q^{-1} S,
\end{equation}
where $Q$ is the covariance matrix of measurement error~\cite{cintron-arias_sensitivity_2009, balsa-cantoComputationalProcedures2008}.
...
\end{equation}
Note that this approach is heuristic and the matrix $F$ is not the \ac{fim}~\cite{versyckIntroducingOptimal1999, banks_generalized_2010}.
%
% ########################################################################
...
The \ac{fim} needs to be mapped on an objective function. There are several ways to do this resulting in different objectives also called criteria.
...
For more information the reader should turn to the available literature, e.g.,~\cite{stornDifferentialEvolutionSimple1997, walesGlobalOptimisationBasinHopping1997}.
\newline
...
The second implemented method is basin-hopping developed by David Wales and Jonathan Doye~\cite{walesGlobalOptimisationBasinHopping1997}.
...
\newline
Some other methods for the optimal control problem the reader can find, for example, in the papers of Banga \etal \cite{banga_global_1996, banga_dynamic_2005, bangaImprovingFoodProcessing2003}.
%
% ########################################################################
...
The deafault arguments of the \mintinline[bgcolor=white,style=emacs]{python}["scipy_differential_evolution"] optimisation method are given by the scipy default arguments~\cite{scipydiffev}.
For our specific usecase, we advise to experiment with the following options:
...
\cmt{?? For the studied model the mentioned above identifiability condition is satisfied if at least two temperatures and in total three timepoints are measured.}
...
The algorithm has shown that at least two different temperarures are required, optimal values for which are $3.5^\circ$C and $12^\circ$C.
Two different timepoints were chosen for temperature $12^\circ$C and  one time point for temperature value $3.5^\circ$C.
There results can be explained with local sensitivity curves where each of the chosen time points correspond to an extreme of at least one sensitivity (see Figure~\ref{fig:baranyi_roberts_sensitivities}).
...
Here we see that one measurement time is not enough as the determinant is numerically very close to $0$ indicating a non-identifieability of this systems configuration.
%
%
\begin{figure}[H]
    \centering
