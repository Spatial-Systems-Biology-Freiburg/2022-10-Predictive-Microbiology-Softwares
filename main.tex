...
Here $x_1^\text{max}, b, T^\text{min}$ are the parameters that we estimate using observational data $y$ at measurement times $t_i$, and temperature $T$ is an input of the system.
...
The values of the parameter vector $\mbp = (n^\text{max}, b_x, T^\text{min}_x, b_y, T^{\text{min}, y})$ and the vector of the initial values $\mbx(t_0)=(x_1(t_0), x_2(t_0), y_1(t_0), y_2(t_0))$ are presented in Table~\ref{Table3}.
%
...
\begin{figure}[H]
    ...
    \begin{subfigure}{.5\textwidth}
        \centering
        \subcaption{{\footnotesize $T_1=3.5^\circ$C, parameter $T^\text{min}$}}
        \includegraphics[scale=0.25]{Figures/Fig7e.pdf}
    \end{subfigure}
    \begin{subfigure}{.5\textwidth}
          \centering
          \subcaption{{\footnotesize $T_2=12^\circ$C, parameter $T^\text{min}$}}
          \includegraphics[scale=0.25]{Figures/Fig7f.pdf}
        \end{subfigure}
    \caption{The example of the sensitivities of the total bacterial count calculated for the experimental design optimization procedure for the Baranyi-Roberts model.
    The line plot presents the model solution for the observable, and the circles determine the time points suggested by experimental design.
...
\begin{figure}[H]
    ...
    \caption{The determinant of the \ac{fim} as a function of the number of measurement times for the experimental design consisting of two measured temperatures.}
    \label{Fig8}
\end{figure}
%
%
...
\begin{table}[H]
    ...
    The values correspond to estimated parameters for the Baranyi-Roberts model for Pseudomonas spp. in poultry under aerobic conditions~\cite{gospavic_mathematical_2008}.}
    \label{Table2}
\end{table}
%
%
\begin{table}[H]
    ...
    \bottomrule
    \end{tabular}
    \caption{The list of the parameter and \ac{ode}'s initial values of the system that fully define the Baranyi-Roberts two-species system~(\ref{eq:model_2species_resource}).}
    \label{Table3}
\end{table}
%
% ########################################################################
